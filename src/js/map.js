...
function setListView(search){
  ...
  for (const area in grouped) {
    ...
    grouped[area].forEach(shop => {
      ...
      label = (shop.member != 1) ? '' : '<div class="search-list__label">会員</div>';
      // console.log(shop.name+" = "+shop.member);
      // console.log(shop.member != 1);
      // console.log(member);
      // console.log("----");

      list += '<div class="search-list__item" id="'+url_id+'" data-member="'+member+'">';
      list += '<div class="search-list__header">';
      list += '<div class="search-list__name">'+shop.name+label+'</div>';
      list += '</div>';
      list += '<div class="search-list__cont">';
      list += '<div class="search-list__title">住所</div>';
      list += '<div class="search-list__data search-list__data--adr">';
      list += '<a href="https://www.google.com/maps/search/?api=1&query='+map_txt+'" target="_blank" class="search-list__map-link">';
      list += '〒'+shop.zip+' '+shop.adr;
      // list += '<img src="/common/images/ico_gmap.svg" alt="" class="search-list__map-icon" />';
      list += '</a>';
      list += '</div>';
      list += '<div class="search-list__title">電話番号</div>';
      if (shop.tel2 === "" || shop.tel === shop.tel2) {
        list += '<div class="search-list__data"><a href="tel:'+tel_link(shop.tel)+'">'+shop.tel+'</a></div>';
      } else {
        list += '<div class="search-list__data"><a href="tel:'+tel_link(shop.tel)+'">'+shop.tel+'</a>　<a href="tel:'+tel_link(shop.tel2)+'">'+shop.tel2+'</a>（時間外）</div>';
      }
      if (shop.fax) {
        list += '<div class="search-list__title">FAX</div>';
        list += '<div class="search-list__data">'+shop.fax+'</div>';
      } 
      if (shop.open) {
        list += '<div class="search-list__title">開局時間</div>';
        list += '<div class="search-list__data">'+shop.open+'</div>';
      } 
      if (shop.teikyuu) {
        list += '<div class="search-list__title">定休日</div>';
        list += '<div class="search-list__data">'+shop.teikyuu+'</div>';
      } 
      list += '</div>';
      list += '</div>';
    });
    ...
    if (list != "") {
      item += '<h2 class="search-list__area">' + area + '</h2>'+list;
    }
  }
  if (!item) {
  ...
  //   label = (this.member == 1) ? '<div class="search-list__label">会員</div>' : '';


  //   item += '<div class="search-list__item" id="'+url_id+'">';
  //   item += '<div class="search-list__header">';
  //   item += '<div class="search-list__name">'+this.name+label+'</div>';
  //   item += '</div>';
  //   item += '<div class="search-list__cont">';
  //   item += '<div class="search-list__title">住所</div>';
  //   item += '<div class="search-list__data search-list__data--adr">';
  //   item += '<a href="https://www.google.com/maps/search/?api=1&query='+map_txt+'" target="_blank" class="search-list__map-link">';
  //   item += '〒'+this.zip+' '+this.adr;
  //   // item += '<img src="/common/images/ico_gmap.svg" alt="" class="search-list__map-icon" />';
  //   item += '</a>';
  //   item += '</div>';
  //   item += '<div class="search-list__title">電話番号</div>';
  //   if (this.tel === this.tel2) {
  //     item += '<div class="search-list__data"><a href="tel:"'+tel_link(this.tel)+'">'+this.tel+'</a></div>';
  //   } else {
  //     item += '<div class="search-list__data"><a href="tel:"'+tel_link(this.tel)+'">'+this.tel+'</a>　<a href="tel:"'+tel_link(this.tel2)+'">'+this.tel2+'</a>（時間外）</div>';
  //   }
  //   if (this.fax) {
  //     item += '<div class="search-list__title">FAX</div>';
  //     item += '<div class="search-list__data">'+this.fax+'</div>';
  //   } 
  //   if (this.open) {
  //     item += '<div class="search-list__title">開局時間</div>';
  //     item += '<div class="search-list__data">'+this.open+'</div>';
  //   } 
  //   if (this.teikyuu) {
  //     item += '<div class="search-list__title">定休日</div>';
  //     item += '<div class="search-list__data">'+this.teikyuu+'</div>';
  //   } 
  //   item += '</div>';
  //   item += '</div>';
...
$(function(){
  ...
  $select.on("click",function(){
    ...
    setListView(val);
    $("html,body").animate({scrollTop:$('#main_in').offset().top});
    // setListView(0);
  });
});
